...
{
  iter = iter+1; 
  input = dxl.getPresentPosition(DXL_ID);                //read from rotary encoder connected to A0
  c_position[iter]=input;
...
//  DEBUG_SERIAL.print("PID Output : ");  
//  DEBUG_SERIAL.println(output); DEBUG_SERIAL.println();
  dxl.setGoalCurrent(DXL_ID, output);                //control the motor based on PID va
  delay(10);
  
  if(dxl.setGoalPosition(DXL_ID, setPoint)){
    delay(10);
...
//    DEBUG_SERIAL.println(dxl.getPresentCurrent(DXL_ID)); DEBUG_SERIAL.println();
  ...
  if (iter==50){
    for(int i=0;i<50;i++){
      DEBUG_SERIAL.println(c_position[i]);     }
  }
}
...
//  DEBUG_SERIAL.print("setPoint : ");  
//  DEBUG_SERIAL.println(setPoint); DEBUG_SERIAL.println(); 
//  DEBUG_SERIAL.print("Error : ");  
//  DEBUG_SERIAL.println(error); DEBUG_SERIAL.println(); 
